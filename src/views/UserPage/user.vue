  ...
  <div>
    <v-card class="overflow-hidden mx-auto rounded-0">
      <v-bottom-navigation  class="rounded-0" color="primary" flat absolute shift grow transparent>
        <v-btn to="/user/dashboard"><span>首页</span><v-icon>mdi-home</v-icon></v-btn>
        <v-btn v-if="id==='0'" to="/user/people"><span>人员管理</span><v-icon>{{mdiAccountTie}}</v-icon></v-btn>
        <v-btn  v-else to="/user/custmess"><span>业主信息</span><v-icon>{{mdiAccountCircle}}</v-icon></v-btn>
        <v-btn v-if="id==='0'" to="/user/other"><span>其他事务</span><v-icon>{{mdiHammerScrewdriver}}</v-icon></v-btn>
        <v-btn v-if="id==='1'" to="/user/charge"><span>收费管理</span><v-icon>mdi-cash-100</v-icon></v-btn>
        <v-btn v-if="id==='1'" to="/user/fix"><span>报修管理</span><v-icon>{{mdiHammerScrewdriver}}</v-icon></v-btn>
        <v-btn v-if="id==='1'" to="/user/poster"><span>公告管理</span><v-icon>{{mdiPost}}</v-icon></v-btn>
      </v-bottom-navigation>
      <!--TODO 获取当前设备的高度替换812        -->
...
export default {
  data (){
    return{
      ...
      mdiHammerScrewdriver:mdiHammerScrewdriver,
      mdiPost:mdiPost,
