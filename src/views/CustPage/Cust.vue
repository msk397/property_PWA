...
import { mdiEmailMarkAsUnread,mdiEmailOpen,mdiHammerScrewdriver} from '@mdi/js';
export default {
  data (){
    return{
      ...
      drawerDisplay: null,
      mess:{real:"", phone:"", addr:"",login:window.localStorage.getItem('loginname')},
  ...
  methods:{
    ...
    query:function (){
      this.axios.post(this.url+'user/querycustmess', JSON.stringify({"name":window.localStorage.getItem('loginname')}),
      ).then(res => {//true
